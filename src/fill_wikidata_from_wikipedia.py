...
def fill_wikidata_from_wikipediacommand(area, dry_run, filters, username, verbose):
    ...
    try:
        ...
        if changeset and not dry_run:
            print(f'DONE! {n_edits} objects modified from {n_objects_with_wikidata}'
                  f' objects with available translations ({round(n_edits / n_objects_with_wikidata * 100)}%)'
