...
@click.option('--output-format', type=click.Choice(['csv', 'mediawiki'], case_sensitive=False), help='Format of the output file.')
...
def translate_with_wikidatacommand(area, dry_run, cache_answers, filters, lang, output, output_format, username, verbose):
    ...
    if verbose > 1:
        print(str(len(result.nodes)) + ' nodes; ' + str(len(result.ways)) + 'ways; ' + str(len(result.relations)) + ' relations')
            ...
            if cache_answers and db[translations['id']]['answer']['committed']:
                ...
                if translations['translations']:
                    ...
                    if translation_options:
                        ...
                        select_translation = '-'
                        print(Fore.YELLOW + "SKIP: WRONG path not translations['translations']? No translations from wikidata." + Style.RESET_ALL)
                    ...
                    select_translation = '-'
                    print(Fore.RED + 'SKIP: No translations from wikidata.' + Style.RESET_ALL)
